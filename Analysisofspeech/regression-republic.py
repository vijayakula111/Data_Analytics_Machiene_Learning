...
try:
		...
		for filename in items:
			...
			#for filename in pathlist:
					...
					#print(open_file)
					republic_speaker = ngrams(open_file,2)
					#print(republic_speaker)
					with open('dbfiles/2000-republic-tokens.csv','r+') as csvfile:
    						csvdemocdata = csv.reader(csvfile)
    						for row in csvdemocdata:
    							content = list(row[i] for i in [0])
							speech_tokens=speech_tokens+content
							#print(content[0])
					print('++++++++++++++++++++++++++++++++++++++++')
					#print(type(republic_speaker))
					for token in republic_speaker.keys():
						#print(token,"token")
						#print(speech_tokens,"speech_tokens")
						if token in speech_tokens:
							#print(token,republic_speaker[token])

							v = republic_speaker[token]/fnotjr
							#print(v,"v")
					...
					if len(response_list)>0 and len(predictor_list)> 0:
						...
						'''with open('dbfiles/republican_congressman_individual_metrics_inline.csv', 'a+') as csvfile:
